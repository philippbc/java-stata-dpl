...
{pstd}
Additionally, {cmd:jcd} maps post-estimation scalars and macros, which aren't accessible from within Java plugins, to general use scalars and macros, which are accessible from within Java plugins. That is, all {cmd:e(X)} macros and scalars are accessible as {cmd:e_X}, e.g. {cmd:e(cmd)} as {cmd:e_cmd}; same for {cmd:r(X)}.
...
{pstd}
In a first step, the {it:Dynamic Plugin Loader} (DPL) has to be configured. Think of it as setting up your IDE by configuring the CLASS_PATH to a 3rd party library. DPL expects config/dpl.xml in the current user.dir. In the case of Stata, this is always the working directory ({cmd:pwd}) at the time the Java Virtual Machine (JVM) was started.
...
{pstd}
In order for DPL to load a plugin dynamically, the plugin has to implement the {cmd:Plugin} interface. {cmd:Plugin.execute(String[])} is the {it:instantiated} entry point for the plugin. The execution environment in {cmd:Plugin.execute(String[])} is identical to Stata invoking a static entry point. That is, everything you can access via the SFI API (e.g., varlist, if, in) and the supplied arguments are identical. See {cmd:Plugin}'s documentation for more details.

{marker remarks}{...}
{title:Remarks}

{pstd}
If your plugin class is on one of the ADO paths DPL will always use this version; it will not load your class from the resource paths specified in the configuration file. That is, versions of classes on ADO paths supersede all versions on CLASS_PATHs and JAR_PATHs.
...
{pstd}
You can run DPL with Java 8 (e.g., if your plugin needs Java 8). To do so, {cmd:set java_vmpath "C:\Program Files\Java\jre1.8.0_XX\bin\server\jvm.dll"} will do the trick (replace XX with your current Java 8 version). For details, see {cmd:query java}.
